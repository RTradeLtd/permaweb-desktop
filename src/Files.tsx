...
class Files extends Component<FileListProps> {
  ...
  setSelectedFileByPosition(index: number) {
    ...
    } else if (fileIds.length > 0 && index >= 0 && index < fileIds.length) {
  ...
  render() {
      ...
      view = (
        <div>
          <p title="folder">
            ...
            <Icon name="file" />
            Showing History: {selectedFile.length} versions
          </p>
          <hr></hr>
          <input
    ...
    return (
      <Shortcuts name="FILE_LIST" handler={this._handleShortcuts}>
        <Card style={{ width: '100%' }}>
          ...
          <Card.Content>
            <Card.Description>{view}</Card.Description>
          </Card.Content>
          <Card.Content extra>
